...
if (isset($_SESSION['user']) && isset($_GET['toggle_fav'])) {
    ...
    if ($check->fetch()) {
        $pdo->prepare("DELETE FROM favorites WHERE user_id = ? AND content_type = 'movie' AND content_id = ?")->execute([$user_id, $movie_id]);
    } else {
        $pdo->prepare("INSERT INTO favorites (user_id, content_type, content_id) VALUES (?, 'movie', ?)")->execute([$user_id, $movie_id]);
    }
            ...
            <?php else: ?>
                <?php foreach ($movies as $movie): ?>
                    <div class="col-md-3 mb-4">
                        <div class="card hover-card movie-card" data-id="<?=$movie['id']?>">
                            <?php if ($movie['poster_path']): ?>
                                <a href="movie_detail.php?id=<?=$movie['id']?>&lang=<?=$lang?>">
                            ...
                            <div class="card-body">
                                <h5 class="card-title">
                                    <a href="movie_detail.php?id=<?=$movie['id']?>&lang=<?=$lang?>"><?=$movie[$lang == 'fr' ? 'title' : 'title_ru'] ?? $movie['title']?></a>
                                </h5>
                                ...
                                <?php if (isset($_SESSION['user'])): ?>
                                    ...
                                    $fav = $is_fav->fetch();
                                    ?>
                                    <a href="?toggle_fav=<?=$movie['id']?>&lang=<?=$lang?>&page=<?=$page?>&genre=<?=$genre_filter?>&search=<?=$search?>" class="btn btn-sm <?=$fav ? 'btn-danger' : 'btn-success'?>">

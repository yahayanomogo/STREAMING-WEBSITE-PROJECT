...
$translations = [
    'fr' => [
        'favorites_title' => 'Ma liste',
        'no_favorites' => 'Aucun élément dans la liste.',
        'remove' => 'Retirer',
        'back' => 'Retour'
    ],
    'ru' => [
        'favorites_title' => 'Мой список',
        'no_favorites' => 'Нет элементов в списке.',
        'remove' => 'Удалить',
        'back' => 'Назад'
    ]
];
...
if (isset($_GET['remove'])) {
    ...
    header('Location: ma_liste.php?lang=' . $lang);
    exit;
}

$stmt = $pdo->prepare("
    SELECT f.id, f.content_type, f.content_id, m.title, m.title_ru, m.poster_path, m.genre, s.title as s_title, s.title_ru as s_title_ru, s.poster_path as s_poster, s.genre as s_genre
    FROM favorites f
                ...
                <?php foreach ($favorites as $fav): ?>
                    <div class="col-md-4 mb-4">
                        <div class="card" style="position: relative;">
                            ...
                            $genre = $fav['content_type'] == 'movie' ? $fav['genre'] : $fav['s_genre'];
                            ?>
                            <?php if ($poster): ?>
